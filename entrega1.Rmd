...
```{r}
names(school_enrollment) = c("Pais", "Porcentaje de jovenes matriculados")

```

Mi columna numérica (la del porcentaje) tiene muchos decimales; lo redondeo a solamente dos
```{r}
school_enrollment$`Porcentaje de jovenes matriculados` = round(school_enrollment$`Porcentaje de jovenes matriculados`, 2)
```

Existen algunos N.A que no me interesa conservar; los elimino
```{r}
...
Primero descargo la libreria rio para poder realizar la funcion importar
```{r}
install.packages("rio")
library(rio)
linkGDP = "https://github.com/marianareinosov/trabajo-final-est/raw/master/Results.csv"
...
```{r}
names(dataGDP)
names(dataGDP) = c("Pais","Año","unidad","Ingreso per capita (en dolares)")
```
...
Redondeo mi tercera columna a 2 decimales
```{r}
dataGDP$`Ingreso per capita (en dolares)` = round(dataGDP$`Ingreso per capita (en dolares)`, 2)
```
...
```{r}
names(Pobreza) = c("Pais", "Poblacion bajo la linea de pobreza nacional (porcentaje)")
names(Pobreza)
```
Para limpiar la base de datos, solo extraigo el número de la columna "Poblacion bajo la linea de pobreza nacional (porcentaje)". 
Utilizo la función parse number, para la cual descargo primero la libreria readr
```{r}
install.packages("readr")
library(readr)
Pobreza$`Poblacion bajo la linea de pobreza nacional (porcentaje)` = parse_number(Pobreza$`Poblacion bajo la linea de pobreza nacional (porcentaje)`)
